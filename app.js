...
const RestaurantCard = (props) => {
  console.log(props);
  let cus = "";
  for (let i = 0; i < Math.min(props.resturant.info.cuisines.length, 3); i++) {
    cus += props.resturant.info.cuisines[i];
    cus += " ";
  }
  return (
      ...
      <div className="card">
        <div className="card-media">
          ...
          <div className="discount">
            {props.resturant.info?.aggregatedDiscountInfoV3?.header}
            <p>{props.resturant.info?.aggregatedDiscountInfoV3?.subHeader} </p>
          </div>
          <div className="pro-discount">PRO extra 15% OFF</div>
          <div className="delivery-time">
            {" "}
            {props.resturant.info?.sla?.deliveryTime + " "}
            mins
          </div>
        ...
        <div className="card-description">
          <div className="about-place">
            <div className="place">
              <p className="place-name">{props.resturant.info.name}</p>
              <p className="place-speciality">{cus}</p>
            </div>
            <div className="place-review">
              <p className="rating">
                {props.resturant.info?.avgRatingString} ★
              </p>
              <p className="per-person">
                {props.resturant.info?.costForTwo} per person
              </p>
            </div>
          </div>
        </div>
...
const Body = () => {
  return (
    <div className="restaurant-list">
      {restaurants.map((rr) => {
        return <RestaurantCard resturant={rr} />;
      })}
    </div>
  );
};
